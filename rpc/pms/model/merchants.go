...
type (
	MerchantsModel interface {
		...
		GetMerchantsByID(id int64) (user *Merchants, err error)
	}
...
func (m *defaultMerchantsModel) GetMerchantsList(in *pms.MerchantsListReq) ([]*Merchants, int64, error) {
	...
	if in.PrincipalPhone != "" {
		db = db.Where("principal_name LIKE ?", fmt.Sprintf("%%%s%%", in.PrincipalPhone))
	}
	if in.PrincipalName != "" {
...
func (m *defaultMerchantsModel) GetMerchantsByID(id int64) (user *Merchants, err error) {
	err = m.conn.Model(&Merchants{}).Where("id=?", id).First(&user).Error
	if err != nil {
		return nil, err
	}
	return user, nil
